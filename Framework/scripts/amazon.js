var alpha = (function ( alpha, $ ) {
	...
	alpha.amazon = function (current_click) { 
		...
		$.post(
			...
			function (response) { 
				klass.display_books(response, current_click.element);
			},
			'json'
		);
	};

	alpha.amazon.prototype.display_books = function (items_to_display, element) { 
		...
		var books_to_display = this.return_books(items_to_display);
			...
			function (book_key, book) { 
				...
				element.after(
					...
					'<div class="book_display_weight"> Weight : '    + book.weight      +'</div>'+
					'<div class="book_display_dimensions"> Dimensions : '+ book.length +'x'+ book.width +'x'+ book.height +'</div></div></div>'
				);
			});
	};
